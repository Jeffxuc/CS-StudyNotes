...
2. 线程的调度算法
   > 1. **先来先服务（First-Come, First-Served，FCFS）**： 按线程到达的顺序进行服务。
   > 2. **短作业优先**：非抢占式的调度算法，按估计运行时间最短的顺序进行调度。
   > 3. **最短剩余时间优先**: 按剩余运行时间的顺序进行调度
   > 4. **优先级调度**：为每个线程分配优先级，根据优先级来进行调度
   > 5. **时间片轮转**：每个线程按照时间片轮流执行，时间用完之后线程被暂停并被放到就绪队列的末尾。
   > 6. **多级反馈队列**：将就绪队列分成多个优先级队列，每个队列有不同的时间片大小。线程首先进入最高优先级队列，如果时间片用完仍未完成，则降低优先级并进入下一级队列。
3. 多线程要注意哪些事项：
...
4. 两个进程要通信，有哪些方式，什么样的方式最快？ :star::star::star::star:
   > 1） 管道。 2）消息队列。3）信号量。4）共享内存。5）套接字Socket。6）RPC。 其中共享内存是不同进程之间直接操作一块内存区域，没有数据的拷贝操作，速度相对较快
...
14. hash表的底层实现？
15. C++的右值引用，左值问题
      > 可以取地址的、有名字的就是左值，反之，不能取地址的、没有名字的就是右值（将亡值或纯右值）。举个例子，int a = b+c, a 就是左值，其有变量名为a，通过&a可以获取该变量的地址；表达式b+c、函数int func()的返回值是右值，在其被赋值给某一变量前，我们不能通过变量名找到它，＆(b+c)这样的操作则不会通过编译。
16.  指针引用区别
17.  new和malloc区别
18.  map和hashmap的区别
19.  虚函数怎么调用的
...
49.  C/C++程序编译的整个过程
      > 1. **预处理**: 1）将所有#include指令包含的头文件插入到源代码中。2）扩展所有 #define声明指定的宏。3）处理条件编译指令,决定需要编译的源码部分。4）移除注释。5）对源码进行重新编号。6）预处理器可以报告各种语法错误。
      > 2. **编译**： 将源文件编译成汇编代码
...
6. socket编程中read失败可能的原因有哪些
   ...
   >4. 其它各种问题，如数据格式，权限问题，软件Bug等。
   > read函数的返回值，可能为大于0：表示读取的字节数；等于 0 表示对方已经关闭了socket；等于-1，此时要查看 errno, 来判断具体的原因，常见的有1）EINTR 表示当前系统中断，2）EAGAIN 或 EWOULDBLOCK 可能是非阻塞socket暂时未读到数据。3）EINVAL：表示传入的参数无效，比如缓冲区指针为空或者长度为负数。4）EMFILE或ENFILE：表示系统资源不足，无法打开更多的文件描述符。
...
1. 软件开发方式有哪些：
   1. 瀑布式开发：
      > 瀑布模型是一种线性顺序的开发模型，包括需求分析、设计、编码、测试和维护等阶段。每个阶段在前一个阶段完成后开始，并且没有交叉迭代。瀑布模型适用于需求稳定、项目规模较小、技术可行性高的项目。 
   2. 迭代开发：
      > 增量模型将系统划分为多个功能模块，每个模块独立开发和测试，并逐步集成形成完整的系统。每个增量都是一个可执行的部分系统，从而可以更早地交付和获取反馈。增量模型适用于需求变化频繁、项目规模较大、风险管理较为重要的项目。 
   3. 螺旋式开发
      > 螺旋模型将瀑布模型和迭代模型相结合，通过循环迭代的方式逐步完善系统。每个迭代包括风险分析、原型开发、评估和计划下一迭代等阶段。螺旋模型适用于大型、复杂且风险较高的项目。 
   4. 敏捷开发:
      > 敏捷模型是一种迭代增量的开发方法，强调团队合作、快速交付、持续反馈和适应变化。代表性的敏捷方法包括Scrum、Kanban和极限编程（XP）。敏捷模型适用于需求变化频繁、团队协作重要的项目。
2. 软件开发流程
3. 常见设计模式
